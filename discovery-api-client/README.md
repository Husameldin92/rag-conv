...
**Config:** `.env` — `GRAPHQL_ENDPOINT`, `AUTH_TOKEN`. 
...
| npm script | File | Purpose |
|------------|------|---------|
| `npm run batch-discovery-llm` | `src/batch-discovery-llm-answers.js` | For **each** question in a JSON file: call **`discovery` only**, download **LLM answer** from `streamUrl`, write `reports/discovery-llm-answers-{timestamp}.csv`. **Formats:** (1) `["q1", "q2"]` or (2) `[{ "lang": "en", "text": "..." }, ...]` — **one question per row**, each tagged with the language you ask in (no repeated translations of the same prompt). Default: `fixtures/questions.json`. Multilang: **`fixtures/questions-multilang.json`** is generated by `npm run compose-multilang` from **`fixtures/questions.json`** + **`fixtures/de.txt`** + **`fixtures/nl.txt`** (103 lines each, same order). Row *i* uses `en` / `de` / `nl` in rotation and the matching language line. Edit `de.txt` or `nl.txt`, then re-run `compose-multilang`. Optional: `--limit N`. **~5s delay** between questions. |

**When to use:** You need the **full LLM text** for many questions via the 3K (`discovery`) path — not just POC rows in the standard report.
...
- `reports/discovery-llm-answers-{timestamp}.csv` (from `batch-discovery-llm`)
