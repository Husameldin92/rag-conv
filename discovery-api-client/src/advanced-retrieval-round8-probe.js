/**
 * BACKEND-1602 QA — ROUND 8 probe (read-only, PRODUCTION).
 *
 ...
 *
 * New file for ROUND 8 — does NOT modify any existing script. Native fetch (node-fetch hangs on v26).
 * access-token header from .env AUTH_TOKEN. Read-only, low + spaced volume.
 * Usage: node src/advanced-retrieval-round8-probe.js
 */
import fs from 'fs';
...
// R7 no-Q single-total baseline (for the ~5× jump re-measure)
const R7_NOQ = { Conference: 2956, Tutorial: 78, Article: 2423, 'Live Event': 152, Camp: 721, Attended: 1275, Favorited: 2, Continue: 10 };
const R7_WITHQ = { Conference: 52, Tutorial: 97, Article: 65, 'Live Event': 71, Camp: 89, Attended: 27, Favorited: 1, Continue: 3 };
...
const OUT = { capturedAt: new Date().toISOString(), endpoint: ENDPOINT, round: 8, schema: null, harvest: null, singles: {}, combines: {}, regression: {}, totalsJump: {}, calls: 0 };
...
(async () => {
  ...
  console.log('\n=== ~5× NO-Q TOTALS RE-MEASURE (vs R7) ===');
  for (const nm of NAMES) {
    const now = A(nm, 'noQ').tc, r6 = R7_NOQ[nm];
    const ratio = r6 ? (now / r6) : null;
    OUT.totalsJump[nm] = { noQ_now: now, noQ_R7: r6, ratio: ratio ? +ratio.toFixed(2) : null, withQ_now: A(nm, 'withQ').tc, withQ_R7: R7_WITHQ[nm] };
    console.log(`  ${nm.padEnd(11)} noQ now=${String(now).padStart(6)}  R7=${String(r6).padStart(6)}  ratio=${ratio ? ratio.toFixed(2) + '×' : 'n/a'}   |  withQ now=${A(nm, 'withQ').tc} (R7 ${R7_WITHQ[nm]})`);
  }
  ...
  const p = path.join(dir, `advanced-retrieval-round8-${stamp}.json`);
